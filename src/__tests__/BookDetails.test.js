...
jest.mock('react-router-dom', () => ({
  ...
  useParams: () => ({ id: '1' }), // Мокаем ID из URL
...
const mockBook = {
  ...
  publicationDate: '1925-04-10T00:00:00.000Z', // ISO формат даты
  isbn: '9780743273565',
  genre: 'Classic',
  availableCopies: 3,
  totalCopies: 5,
...
describe('BookDetails Component', () => {
  ...
  test('displays error message if fetching fails', async () => {
    ...
    error.response = { data: { message: 'API Error Fetching' } };
  ...
  const openDeleteModal = async () => {
      await screen.findByRole('heading', { name: mockBook.title }); // Убедимся, что страница загрузилась
  ...
  test('opens delete confirmation modal when Delete button is clicked', async () => {
    ...
    expect(screen.getByRole('button', { name: /Delete/i, container: modal })).toBeInTheDocument(); // Кнопка Delete в модалке
  });
  ...
  test('shows specific API error toast if deleteBook fails from modal', async () => {
    ...
    expect(screen.getByRole('dialog')).toBeInTheDocument(); // Модалка не закрывается при ошибке
  });
